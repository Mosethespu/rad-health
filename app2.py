...
def deregister_patient():
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        close_db(conn)
        message = f"Patient with ID {patient_id} de-registered."
...
def check_patient():
    ...
    if patient_identifier:
        conn = get_db()
        cursor = conn.cursor()
        # Try searching by ID first, then by date of birth (you might need to refine this)
        cursor.execute("SELECT * FROM patients WHERE patient_id = ?", (patient_identifier,))
        patient_details = cursor.fetchone()
        if not patient_details:
            cursor.execute("SELECT * FROM patients WHERE date_of_birth = ?", (patient_identifier,))
            patient_details = cursor.fetchone()

        if patient_details:
            cursor.execute("SELECT * FROM appointments WHERE patient_id = ?", (patient_details['patient_id'],))
